...
const DmartScraper = () => {
  ...
  const pollTaskStatus = (taskId) => {
    ...
    pollIntervalRef.current = setInterval(async () => {
      try {
        ...
        // Format log lines into state logs
        let parsedLogs = [];
        if (logLines.length > 0) {
          parsedLogs = logLines.map((line) => {
            ...
            // Check if line matches standard Python logging output
            // format: 2026-05-28 19:13:26,556 | INFO     | name : message
            ...
            if (parts.length >= 3) {
              ...
              if (levelPart === "ERROR" || levelPart === "CRITICAL") {
                ...
                logType = "success";
              } else if (contentPart.includes("Stage") || contentPart.includes("START") || contentPart.includes("Initializing")) {
                logType = "system";
              }
            } else {
              // Parse unstructured lines (e.g. standard print or error tracebacks)
              if (line.toUpperCase().includes("ERROR") || line.toUpperCase().includes("FAILED") || line.toUpperCase().includes("EXCEPTION") || line.toUpperCase().includes("HALTED")) {
                logType = "error";
              } else if (line.toUpperCase().includes("SUCCESS") || line.toUpperCase().includes("COMPLETED")) {
                logType = "success";
              } else if (line.toUpperCase().includes("SKIP") || line.toUpperCase().includes("WARNING") || line.toUpperCase().includes("ALREADY FULLY SCRAPED")) {
                logType = "warning";
              } else if (line.startsWith("===") || line.includes("Processing:") || line.includes("Starting") || line.startsWith("[")) {
                logType = "system";
              }
            }
        ...
        // Display parsed logs if they are available
        if (parsedLogs.length > 0) {
          setLogs(parsedLogs);
        }

        if (task.status === "COMPLETED") {
          const finalMsg = `[SUCCESS] D-Mart scraping batch completed successfully! Total Ingested: ${task.total_leads} products.`;
          ...
          const finalMsg = `[ERROR] Execution halted: ${errMsg}`;
          const finalTime = new Date().toLocaleTimeString();
          setLogs((prev) => {
            const hasFinal = prev.some((l) => l.message.includes("Execution halted:"));
            if (hasFinal) return prev;
          ...
          const finalMsg = `[SYSTEM] Scraper halted: stopped by user action.`;
          const finalTime = new Date().toLocaleTimeString();
          setLogs((prev) => {
            const hasFinal = prev.some((l) => l.message.includes("stopped by user action"));
            if (hasFinal) return prev;
            return [...prev, { timestamp: finalTime, message: finalMsg, type: "warning" }];
          });
          setLoading(false);
          if (pollIntervalRef.current) {
            clearInterval(pollIntervalRef.current);
  ...
  const handleScrape = async (overrideCategories) => {
    ...
    try {
      ...
      addLog(`[CELERY] Task successfully queued in Redis! Task ID: ${response.data.task_id}`, "success");
      addLog(`[CELERY] Job message: ${response.data.message}`, "info");
      
      setResult(response.data);

      // Start actual task status polling
      pollTaskStatus(response.data.task_id);

    } catch (err) {
      console.error("Scraping Error:", err);
      const errMsg = err.response?.data?.error || "Failed to trigger D-Mart scraper.";
      setError(errMsg);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl border border-blue-gray-100 shadow-sm">
        <div>
          <Typography variant="h3" color="blue-gray" className="font-bold flex items-center gap-3">
            ...
            D-Mart Automation Scraper
          </Typography>
          <Typography className="text-sm text-gray-500 mt-2 font-medium">
            Deploy an asynchronous, Playwright stealth browser to scrape live groceries pricing, details, and stock records directly from D-Mart Ready.
          </Typography>
        </div>
      </div>
        ...
        <Card className="lg:col-span-5 shadow-lg border border-blue-gray-100 h-fit">
          <CardBody className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              />
              <Typography className="text-[10px] text-gray-400 mt-1">
                Limit categories crawl for rapid configuration verification.
              </Typography>
            </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto font-mono text-xs space-y-2 no-scrollbar">
              {logs.length === 0 ? (
                <div className="text-gray-500 italic h-full flex items-center justify-center">
                  Terminal inactive. Select scraper parameters and click "Start D-Mart Scrape" to watch live execution logs.
                </div>
              ) : (
                logs.map((log, idx) => (
