...
export function CategoriesPendingReport() {
  ...
  const tableRows = useMemo(() => {
    ...
    return list;
  }, [pendingCategories, categorySearch]);
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-20 gap-3">
        ...
        <Typography className="animate-pulse text-gray-600 font-medium">Loading Pending Categories...</Typography>
      </div>
    );
  }
    ...
    <div className="mt-8 mb-12 px-4 max-w-5xl mx-auto">
      <div className="mb-4">
        ...
        <Typography variant="small" className="text-gray-500">
          Professional view of categories that still require mapping.
        </Typography>
      </div>
              ...
              {tableRows.length === 0 ? (
                <tr>
                  <td colSpan={2} className="px-6 py-12 text-center text-gray-400 italic bg-gray-50">
                    No pending categories found matching "{categorySearch}"
                  </td>
                </tr>
              ) : (
                tableRows.map((row) => (
